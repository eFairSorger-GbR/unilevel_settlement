module UnilevelSettlement
  class PayoutInvoice < ApplicationRecord
    ...
    has_many :records, class_name: 'UnilevelSettlement::PayoutRecord',
                       foreign_key: 'unilevel_settlement_payout_invoice_id',
                       inverse_of: :invoice

    has_one_attached :invoice_pdf

    scope :published, -> { joins(:run).where.not(unilevel_settlement_payout_runs: { published_date: nil }) }
